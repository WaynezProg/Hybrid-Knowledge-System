...
## Active Technologies
- Python `>=3.12,<3.13` + existing `typer`, `jsonschema`, `mcp`, `starlette`, `python-slugify`; no mandatory new runtime dependency for 008-012
- 008 local JSON artifacts under `KS_ROOT/llm/extractions/`, separate from authoritative `wiki/`, `graph/`, `vector`, and manifest semantics
...
- 012-source-catalog: `ks source list/show` 唯讀 catalog + `ks workspace register/list/show/remove/use/query`
