...
def test_writeback_ask_yes_commits(
    ...
    monkeypatch.setattr(
        query_command,
        "decide",
        lambda flag, is_tty: Decision(action="commit", status="committed"),
    )
...
def test_writeback_ask_no_declines(
    ...
    monkeypatch.setattr(
        query_command,
        "decide",
        lambda flag, is_tty: Decision(action="decline", status="declined"),
    )
...
@pytest.mark.integration
@pytest.mark.us3
def test_writeback_skips_non_tty_by_default(cli_runner, ingested_for_writeback) -> None:
    result = cli_runner.invoke(app, ["query", "summary Atlas"])
