"""Unit tests for page_tree candidate collector."""
...
def test_summary_match_adds_to_score() -> None:
    ...
    score_title = page_tree_node_score("Atlas risk", node_title_only)
    score_both = page_tree_node_score("Atlas risk", node_with_summary)
...
def test_empty_question_returns_zero() -> None:
    node = _make_node(title="Atlas", summary="Atlas summary")
    score = page_tree_node_score("", node)
    assert score == 0.0


def test_chinese_terms_match() -> None:
    node = _make_node(title="風險登記", summary="專案風險評估")
    score = page_tree_node_score("風險評估", node)
    assert score > 0


def test_returns_candidates_with_summary() -> None:
    node = _make_node(
        node_id="n1",
        title="Risk Register",
    ...
    manifest, tree_store = _mock_manifest_and_store({"risk-tree": tree})
...
def test_skips_nodes_without_summary() -> None:
    node = _make_node(node_id="n1", title="project atlas", summary="")
    tree = _make_tree([node], source_relpath="atlas.txt")
    manifest, tree_store = _mock_manifest_and_store({"atlas-tree": tree})

    candidates, _steps = collect_page_tree_candidates(
        "Atlas",
        tree_store=tree_store,
        manifest=manifest,
    )
...
def test_skips_nodes_with_zero_score() -> None:
    node = _make_node(
        node_id="n1",
        title="Billing API",
    ...
    manifest, tree_store = _mock_manifest_and_store({"billing-tree": tree})
...
def test_caps_at_five_candidates() -> None:
    nodes = [
        _make_node(
            node_id=f"n{i}",
    ...
    manifest, tree_store = _mock_manifest_and_store({"big-tree": tree})
...
def test_score_normalized_and_capped() -> None:
    node = _make_node(
        node_id="n1",
        title="Atlas",
    ...
    candidates, _ = collect_page_tree_candidates(
...
def test_trace_step_always_present() -> None:
    manifest = MagicMock()
    manifest.entries = {}
    tree_store = MagicMock()
...
def test_gracefully_handles_tree_load_failure() -> None:
    manifest = MagicMock()
    entry = MagicMock()
...
def test_evidence_includes_relpath_and_section() -> None:
    candidate = Candidate(
...
def test_evidence_omits_missing_optional_fields() -> None:
    candidate = Candidate(
...
def test_no_evidence_when_relpath_missing() -> None:
    candidate = Candidate(
        text="orphan node",
        source_route="page_tree",
        score=0.2,
