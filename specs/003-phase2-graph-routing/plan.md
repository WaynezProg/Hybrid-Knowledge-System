...
- **§V Write-back Safety**：Archived original plan used CLI default `auto` with CI / smoke / agent workflow explicitly passing `--writeback=no`。2026-05 retrieval-safety hardening supersedes this: current CLI default is `writeback=no` and `auto` requires explicit `--writeback=auto`。**PASS**。
